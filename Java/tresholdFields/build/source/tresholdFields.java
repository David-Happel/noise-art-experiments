...
public class tresholdFields extends PApplet {
...
public void draw_(){
  ...
  for(float x = 1;x<=width;x+=1){
    ...
    for(float y= 1;y<=height;y+=1){
      ...
      boolean white = treshold(v1,v2);
      if(white){
        point(x,y);
      }
      ey += 1f*abs(y-(height/2f))/20;
    }
    ex += 1f*abs(x-(width/2f))/20;
  }
}
  public void settings() {  size(800, 800); }
  static public void main(String[] passedArgs) {
    String[] appletArgs = new String[] { "tresholdFields" };
    if (passedArgs != null) {
