...
def points_for_voronoi(points):
    ...
    return [*points, *right_points, *left_points, *top_points, *bottom_points, *top_left, *top_right, *bottom_left, *bottom_right]
...
def draw(t):
    ...
    for layer in range(layers):
        ...
        points = np.array(
            [[(noise[layer].eval(t, i*100))%1*width, noise[layer].eval(t, (i*100) + 10000)%1*height] for i in range(20)])
...
while not done:
    ...
    if frame >= rec_frames:
        ...
        if recording:
            ...
            img.save(fp="./out/img"+str(frame)+".gif", format='GIF', append_images=imgs, save_all=True, duration=(1/fps)*1000, loop=0, transparency=0, optimize=True, disposal=2)
